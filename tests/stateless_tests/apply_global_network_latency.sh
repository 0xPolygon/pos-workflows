...
run_pumba() {
	...
	docker run -i --rm \
		...
		ghcr.io/alexei-led/pumba:0.12.3 netem "${TARGET_FLAGS[@]}" --tc-image gaiadocker/iproute2 --duration "$NETWORK_LATENCY_DURATION" --interface "$INTERFACE" delay --time "$delay" --jitter "$jitter" "re2:^${prefix}"
}
